...
function setupBookmarklet() {
    const code = `(function(){
/* Phase 1: Nuke copy-protection in main document */
function unlock(doc){
try{
...
/* Phase 5: Cross-origin iframe present — show paste panel */
/* window.getSelection() CANNOT read text from cross-origin iframes (browser security). */
/* Best solution: user does Ctrl+A, Ctrl+C inside the iframe (native browser always allows this), */
/* then pastes into our panel below. We clean and re-copy it. */
if(crossFrames.length>0||document.querySelectorAll('iframe').length>0){
...
ov.innerHTML='<div style="background:#fff;border-radius:12px;padding:24px;width:min(500px,90vw);box-shadow:0 20px 60px rgba(0,0,0,0.4)">'
+'<div style="font-size:17px;font-weight:700;margin-bottom:6px">\\uD83D\\uDD13 Reader detected</div>'
+'<div style="font-size:13px;color:#555;margin-bottom:16px;line-height:1.5">'
+'The text is inside a protected frame. Do this:<br>'
+'<strong>1)</strong> Click inside the reader &nbsp;'
+'<strong>2)</strong> Press <kbd style="background:#f0f0f0;padding:2px 6px;border-radius:4px;border:1px solid #ccc">Ctrl+A</kbd> &nbsp;'
+'<strong>3)</strong> Press <kbd style="background:#f0f0f0;padding:2px 6px;border-radius:4px;border:1px solid #ccc">Ctrl+C</kbd><br>'
+'<strong>4)</strong> Click in the box below and press <kbd style="background:#f0f0f0;padding:2px 6px;border-radius:4px;border:1px solid #ccc">Ctrl+V</kbd>'
+'</div>'
+'<textarea id="_cn_paste" placeholder="Paste your text here..." style="width:100%;height:140px;border:2px solid #2563eb;border-radius:8px;padding:10px;font:14px system-ui;resize:vertical;margin-bottom:12px"></textarea>'
+'<div style="display:flex;gap:8px;justify-content:flex-end">'
+'<button id="_cn_cancel" style="padding:10px 18px;border:1px solid #ddd;border-radius:8px;background:#fff;font:500 13px system-ui;cursor:pointer">Cancel</button>'
+'<button id="_cn_use" style="padding:10px 18px;border:none;border-radius:8px;background:#2563eb;color:#fff;font:600 13px system-ui;cursor:pointer">Copy for Cornell Notes</button>'
+'</div></div>';
...
close();
alert('Copied '+clean.split(/\\s+/).length+' words! Go paste in Cornell Notes.');
}).catch(function(){
close();
prompt('Copy this:',clean);
});
};
}else{
alert('Protection removed! Select the text, then click this bookmarklet again.');
}
})()`;
